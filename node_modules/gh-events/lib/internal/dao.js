...
var getEvent = module.exports.getEvent = function(id, callback) {
    // TODO: Include Organizer users
    DB.Event.find({'where': {'id': id}, 'include': [DB.App]}).complete(function(err, event) {
        if (err) {
...
var getEvents = module.exports.getEvents = function(ids, callback) {
    // TODO: Include Organizer users
    DB.Event.findAll({'where': {'id': ids}, 'include': [DB.App]}).complete(function(err, events) {
        if (err) {
            log().error({'err': err, 'id': id}, 'Failed to get an event');
            return callback({'code': 500, 'msg': err.message});
