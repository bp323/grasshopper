...
var getSeriesEvents = module.exports.getSeriesEvents = function(serie, start, end, limit, offset, includeUpcoming, callback) {
    ...
    var startFilter = startFilter = {'start': {'gte': start}};
    ...
    serie.getEvents(options).complete(function(err, serie) {
        if (err) {
            log().error({'err': err}, 'Failed to get events for a serie');
            return callback({'code': 500, 'msg': err.message});
        }

        return callback(null, serie, serie.Events);
    });
};
 ...
 * @param  {Object}     callback.err    An error object, if any
 */
var removeSeriesEvents = module.exports.removeSeriesEvents = function(serie, events, callback) {
    serie.removeEvent(events).complete(function(err) {
        if (err) {
